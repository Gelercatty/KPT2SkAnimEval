...
from methods.Smplify3D.tools import config
from methods.Smplify3D.tools.smplify import SMPLify3D
from methods.Smplify3D.tools.temos_utils import subsample
...
def plys2npy(ply_dir, out_dir):
    ...
    for item in file_list:
        if item.endswith(".ply") and not item.endswith("_gt.ply") and  not item.startswith("SMPLPose_"):
            paths.append(os.path.join(ply_dir, item))
    if len(paths) == 0:
        raise ValueError(f"No valid .ply files found in {ply_dir}")
    ...
    for i, path in enumerate(paths):
        ...
        vs = mesh.vertices
        assert vs.shape == (6890, 3), f"Mesh shape error in {path}: got {vs.shape}"
        # meshs[i] = vs 
        mesh_list.append(vs)
        
    meshs = np.stack(mesh_list, axis=0)
...
parser.add_argument(
    "--data_folder",
    type=str,
    default="./data/temp",  # ./demo/demo_data/
...
parser.add_argument("--Needmesh", type=bool, default=False, help="true to export the mesh")
...
print(f"begin to trans {len(paths)} npy files!")
...
summerize_dir = os.path.join(opt.save_folder, "result")
os.makedirs(summerize_dir,exist_ok=True)
...
for path in paths:
    ...
    pose_seq = torch.zeros(num_seqs, 72).to(device)
    trans_seq = torch.zeros(num_seqs, 3).to(device)
    for idx in range(num_seqs):
    # for idx in range(0,4,1):## debug
        ...
        ## export mesh, but we dont need the mesh
        if opt.Needmesh:
            mesh_p = trimesh.Trimesh(
        ...
        # save the pose
        pose_seq[idx, :] = new_opt_pose

        trans_seq[idx, :] = new_opt_joints[0, 0, :3]
    AISTpp_style_data = {}
    AISTpp_style_data["smpl_pose"] = pose_seq.cpu().numpy()
    AISTpp_style_data["smpl_trans"] = trans_seq.cpu().numpy()
    # (1,)
    AISTpp_style_data["smpl_scaling"] = np.array([1.0])
    npy_name = os.path.basename(path)[:-4]
    joblib.dump(AISTpp_style_data,
                dir_save + "/"+ "SMPLPose_{}".format(npy_name)+".pkl", compress=3)
    joblib.dump(AISTpp_style_data,
                summerize_dir + "/"+ "SMPLPose_{}".format(npy_name)+".pkl", compress=3) 
    if opt.Needmesh:
