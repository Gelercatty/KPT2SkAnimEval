...
def process_file(path: Path, out_dir: Path, model, device: torch.device, batch: int, apply_scaling: bool = False, joints_mode: str = "24") -> Tuple[Path, Path]:
    raw = _load_any(path)
...
def main():
    ...
    for f in tqdm(files, desc="Converting", unit="file"):
        try:
            npy_path, meta_path = process_file(f, out_root, model, device, args.batch_size, apply_scaling=(not args.ignore_scaling), joints_mode=args.joints)
        except Exception as e:
            print(f"[ERROR] {f}: {e}")
            continue
